  ...
  try {
    ...
    if (await shareButton.isVisible({ timeout: 5000 })) {
      ...
      await page.screenshot({
        path: 'tests/ui/screenshots/read-shares-dialog.png',
        fullPage: true
      });
      console.log('📸 Screenshot saved: read-shares-dialog.png');

      // Check for "Current Shares" section
      const currentSharesSection = page.locator('text=Current Shares, text=Shared Users');
    ...
    await page.screenshot({
      path: 'tests/ui/screenshots/read-shares-error.png',
      fullPage: true
    });
  } finally {
    await browser.close();
