...
async function checkAndAcceptShare() {
  ...
  try {
    ...
    const notificationBell = page.locator(
      'button:has(i.material-icons:text("notifications"))'
    )
    ...
    await notificationBell.screenshot({
      path: path.join(
        __dirname,
        '../screenshots/accept-share-3-notification-bell.png'
      )
    })
    ...
    await page.screenshot({
      path: path.join(
        __dirname,
        '../screenshots/accept-share-4-after-click.png'
      ),
      fullPage: true
    })
    ...
    await firstShare.screenshot({
      path: path.join(
        __dirname,
        '../screenshots/accept-share-6-first-share.png'
      )
    })
    ...
    await page.screenshot({
      path: path.join(
        __dirname,
        '../screenshots/accept-share-7-after-accept.png'
      ),
      fullPage: true
    })
    ...
    for (let i = 0; i < walletCount; i++) {
      ...
      if (text.includes('Joint') || text.includes('Shared')) {
        ...
        await item.screenshot({
          path: path.join(
            __dirname,
            '../screenshots/accept-share-8-shared-wallet.png'
          )
        })
        break
      }
