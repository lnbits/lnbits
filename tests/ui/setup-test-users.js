...
async function setupTestUsers() {
  ...
  try {
    ...
    await createUser(page, config.baseUrl, config.secondaryUsername, config.secondaryPassword);
...
async function createUser(page, baseUrl, username, password) {
  ...
  const loginVisible = await page.locator('text=Login to your account').isVisible({ timeout: 2000 }).catch(() => false);
  ...
  const createAccountVisible = await page.locator('text=Create Account').isVisible({ timeout: 2000 }).catch(() => false);
  ...
  if (createAccountVisible) {
    ...
    console.log(`   ℹ️  User ${username} may already exist or browser already logged in`);
  }
}
