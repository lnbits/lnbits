...
function getConfig() {
  ...
  return {
    baseUrl: config.TEST_LNBITS_URL || process.env.TEST_LNBITS_URL || 'http://localhost:5001',
    ...
    secondaryUsername: config.LNBITS_SECONDARY_USERNAME || process.env.LNBITS_SECONDARY_USERNAME,
    secondaryPassword: config.LNBITS_SECONDARY_PASSWORD || process.env.LNBITS_SECONDARY_PASSWORD,
    walletName: config.TEST_WALLET_NAME || process.env.TEST_WALLET_NAME || 'Test Wallet'
  };
}
...
async function login(page, username = null, password = null) {
  ...
  if (!isLoggedIn) {
    await page.screenshot({ path: 'tests/ui/screenshots/login-verification-failed.png', fullPage: true });
