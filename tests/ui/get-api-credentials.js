  ...
  try {
    ...
    if (apiKey && walletId) {
      ...
      if (envContent.includes('TEST_ADMIN_API_KEY=')) {
        envContent = envContent.replace(/TEST_ADMIN_API_KEY=.*/, `TEST_ADMIN_API_KEY=${apiKey}`);
      } else {
      ...
      if (envContent.includes('TEST_WALLET_ID=')) {
        envContent = envContent.replace(/TEST_WALLET_ID=.*/, `TEST_WALLET_ID=${walletId}`);
      } else {
      ...
      console.log('📝 You can now run the API tests with: cd tests && ./run_api_tests.sh\n');
