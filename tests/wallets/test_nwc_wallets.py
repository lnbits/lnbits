        ...
        async for message in websocket:
            ...
            if msg[0] == "REQ":
                ...
                if 13194 in kinds: # Send info event
                    ...
                    sign_event(mock_settings["service_public_key"], mock_settings["service_private_key"], event)
                    await websocket.send(json.dumps(["EVENT", sub_id, event]))
            elif msg[0] == "EVENT":
                event = msg[1]
                decrypted_content = decrypt_content(mock_settings["service_private_key"], mock_settings["user_public_key"], event["content"])
                ...
                if mock:
                    ...
                    if sub_id:
                        ...
                        encrypted_content = encrypt_content(mock_settings["service_private_key"], mock_settings["user_public_key"], json_dumps(response))
                        response_event = {
                            "kind":23195,
                        ...
                        sign_event(mock_settings["service_public_key"], mock_settings["service_private_key"], response_event)
                        await websocket.send(json.dumps(["EVENT", sub_id, response_event]))
                else:
                    raise Exception("No mock found for "+content["method"]+" "+json_dumps(content["params"]))
...
@pytest.mark.parametrize(
    "test_data",
    wallet_fixtures_from_json(
        "tests/wallets/fixtures/json/fixtures_nwc_bad.json"),
    ids=build_test_id,
)
