    ...
    if lnurl_auth is not None:
        if params is not None and "admin" in params:
            row = await db.fetchone(
                "SELECT * FROM users WHERE lnurl_auth = ? AND admin = ?",
                (lnurl_auth, params["admin"]),
            )
        else:
            row = await db.fetchone(
                "SELECT * FROM users WHERE lnurl_auth = ?", (lnurl_auth)
            )
    else:
...
async def getLogs(usr_id: str, limit: Optional[str]) -> List:
    ...
    row = await db.fetchall(
        f"SELECT * FROM logs WHERE usr = '{usr_id}' ORDER BY time DESC {t}"
    )
    if row is None:
...
async def accountRecovery(params: dict) -> dict:
    try:
        user_exists = await db.fetchone(
            "SELECT * FROM users WHERE lnurl_auth = ?", (params["linking_key"])
        )
        if user_exists is not None:
            return {"error": "Account already exists!"}
        row = await db.execute(
            "UPDATE users SET lnurl_auth = ? WHERE lnurl_auth = ? ",
            (params["linking_key"], params["recovery_key"]),
        )
...
async def handleCredits(id: str, credits: int) -> bool:
    try:
        row = await db.execute(
            f"UPDATE users SET credits = {credits} WHERE id = '{str(id)}'"
        )
        return True
    except ValueError:
        return False
...
async def numPayments(user: str) -> int:
    ...
    count = await db.fetchall(
        f"SELECT COUNT(time) FROM payments WHERE paid = True AND admin_id = '{user}' AND cmd = 'payment' AND time >= {seven_days} ORDER BY time DESC"
    )
