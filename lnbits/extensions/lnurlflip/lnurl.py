...
from .crud import get_lnurlflip_link_by_hash, update_lnurlflip_link


# FOR LNURLs WHICH ARE NOT UNIQUE


@lnurlflip_ext.route("/api/v1/lnurl/<unique_hash>", methods=["GET"])
async def api_lnurl_response(unique_hash):
    link = await get_lnurlflip_link_by_hash(unique_hash)
...
@lnurlflip_ext.route("/api/v1/lnurl/<unique_hash>/<id_unique_hash>", methods=["GET"])
...
@lnurlflip_ext.route("/api/v1/lnurl/cb/<unique_hash>", methods=["GET"])
async def api_lnurl_callback(unique_hash):
    ...
    try:
        ...
        await update_lnurlflip_link(link.id, **changes)
        ...
        await update_lnurlflip_link(link.id, **changesback)
