...
from .crud import get_configuration_by_wallet, create_forward, update_forward
...
async def forward_payment(config: StrikeConfiguration, forward: StrikeForward, payment: Payment) -> bool:
    client = StrikeApiClient()
    tickers = await client.get_tickers()
    target_handle = config.handle
    target_currency = config.currency
    rates = filter(lambda item: item.sourceCurrency == 'BTC' and item.targetCurrency == target_currency, tickers)
    ...
    for spread in safe_spreads:
        ...
        quote = await create_quote(client, payment, target_handle, target_currency, rate, spread)
        ...
        if(converted_sats < payment.sat):
            ...
            print(f'Converted sats {converted_sats} are lower than original sats: {payment.sat} (fee: {forward.msats_fee})')
            break
        print(f'Converted sats {converted_sats} are higher than original sats: {payment.sat}, we need to re-generate quote')
...
async def create_quote(client: StrikeApiClient, payment: Payment, handle: str, currency: str, rate: float, safe_spread: float):
    ...
    invoice_id = await client.issue_invoice(handle, fiat_amount, currency, payment.memo or "From LNbits")
