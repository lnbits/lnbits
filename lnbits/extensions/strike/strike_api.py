...
class StrikeApiClient():
    ...
    async def issue_invoice(self, handle: str, amount: float, currency: str, description: str) -> str:
        ...
        try:
            ...
            self._throw_bad_request(f"Failed to issue Strike invoice, error: '{str(e)}'")
    ...
    def _throw_invalid_api_key(self):
        if(not self.has_api_key):
            raise HTTPException(status_code=HTTPStatus.SERVICE_UNAVAILABLE, detail="Strike API key is not configured, contact administrator")
