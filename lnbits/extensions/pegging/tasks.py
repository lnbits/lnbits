    ...
    while True:
        for c in currencies:
            ...
            for peg in pegs:
                ...
                percent = float(peg.percent) / 100.  # TODO: same, type support
                ...
                if hasattr(ps, symbol):
                    p = getattr(ps, symbol)
                    position_fiat = int(p.quantity) #TODO: type conversions in data types of Kollider client
                ...
                price = float(t.last_price) #TODO: type conversions in data types of Kollider client
                position_msat = int(position_fiat/price * (1_000. * 100_000_000.))

                logger.debug(f"Wallet {peg.wallet} position {position_fiat}{symbol}@{price} - {position_msat} msat")
                ...
                if delta_msat > no_hedge_msat:
                    ...
                    to_zero = int(1./percent*position_msat)
                    logger.debug(
                        f"Winding down {position_msat}msat@{price} {symbol}")
...
async def update_position(pegging: Pegging, delta_amount: int, side: str) -> None:
    ...
    percent = float(pegging.percent) / 100.  # TODO: same, type support
    ...
    if r and "error" in r:
        ...
        logger.debug(f"{pegging.wallet} order placed for {fiat_amount} {symbol} @ {price}")
