...
from falcon import errors, testing
...
EVENTS = [
    {
        ...
        "pubkey": "5faaae4973c6ed517e7ed6c3921b9842ddbc2fc5a5bc08793d2e736996f6394d",
        "created_at": 1672325827,
        "kind": 0,
        "tags": [],
        "content": '{"name": "test1", "nip05": "test@st.germa.in", "description": "test account"}',
        "sig": "bb748c84613eb9073d4dc99fc6df0c96aa43fd380c2c058a71f67a8f0fe012e5c40d74aa81c4c233a092eb307c7b5b8a4499e3403e0a54469ad1db1575916cf5",
    },
    {
        "id": "46981a47c7e28f720a2609a5872c62b6e8b9ae612db3f8320b68be446ad77e11",
        "pubkey": "5faaae4973c6ed517e7ed6c3921b9842ddbc2fc5a5bc08793d2e736996f6394d",
        "created_at": 1672412227,
        "kind": 1,
        "tags": [],
        "content": "hello world",
        "sig": "d81c89008bcb9e27a0fc3454fef6b481034cfeaaf0c48e65299fa9af85e35e2c27e36a2478ba8a402a14ed3c332e4b7a43d7682e9d5baea0a7a9083fc26dba00",
    },
    {
        "id": "ea54cf0e912b3fc482ee842195c8db670df6d958b2ae86e039b7c56690d81ef9",
        "pubkey": "44c7a871ce6224d8d274b494f8f68827cb966e3aaba723a14db8dd22e0542e7d",
        "created_at": 1672325827,
        "kind": 1,
        "tags": [
        ...
        "content": "hello pk1",
        "sig": "73106b28855337fd2546a04d02cec76c1673428811acc42bc2b69cedc03554024b8ca8bb841b9991b728db3be89760486be147b2fb03e3f4e16ca83a356d56ef",
    },
]

REPLACE_EVENTS = [
    {
        "id": "ae0816fff8d0363a189f22cf8ade674c386963cab39d68b28b19a7b5eae90cbd",
        "pubkey": "44c7a871ce6224d8d274b494f8f68827cb966e3aaba723a14db8dd22e0542e7d",
        "created_at": 1672324827,
        "kind": 11111,
        "tags": [],
        "content": "replace me",
        "sig": "0f9bfe1047dfc37a020c8b0196703ec553b22cf9b6258c3e7b4e8a67991fd788813018e90558f18d3337e48152e2e0fa50a8b925e35ad91a8db603c517092f28",
    },
    {
        "id": "2270926268812f4886d4499bc9a29a84223e5554985496843278e1472fab3837",
        "pubkey": "44c7a871ce6224d8d274b494f8f68827cb966e3aaba723a14db8dd22e0542e7d",
        "created_at": 1672326827,
        "kind": 11111,
        "tags": [],
        "content": "replaced",
        "sig": "d48846d5a7d344d60ba66c97ebe923d5f61b2e3970fd92682795ba5ff91137f288fd234ce5cafdaddc9b0f6408085c46c7255994eb3850e99ebbc05e3108f2c6",
    },
]
...
class DBTests(BaseTestsWithStorage):
    ...
    async def test_expiration_tag(self):
        expiring_event = {
            ...
            "created_at": 1672325827,
            "kind": 1,
            "tags": [["expiration", 1672329427]],
            "content": "this will expire",
            "sig": "77509c595b5e4e86a74dc1fa0b4a484a8bca6acc89e2e32b9794c9e0c59a0313521295e8c4b6b844be5ce83bab2cb7230bc0d9761fbf3845a2cd04dab2d70cda",
        }
        distant_future_event = {
            ...
            "created_at": 1672325827,
            "kind": 1,
            "tags": [["expiration", 1987990393]],
            "content": "this will expire in a long time",
            "sig": "046d4ad7eb8b7b64528cfcde2f4171d8ad375467447a134dbd14b764973a237f977c3f8a5d3ba6cfcc5ed07d0fa68b222a18c81c2c9c115bdd55734c7006877b",
        }

        await self.storage.add_event(expiring_event)
        await self.storage.add_event(distant_future_event)
        assert (await self.storage.get_event(expiring_event["id"]))[
            "id"
        ] == expiring_event["id"]
        assert (await self.storage.get_event(distant_future_event["id"]))[
            "id"
        ] == distant_future_event["id"]
    ...
    async def test_ephemeral_event(self):
        ...
        ephemeral_event = {
            ...
            "tags": [],
            "content": "ephemeral",
            "sig": "66f8a055bb3c3fc3fe0ca0ead4d5558d69627dc4f40c7320228d9e4c266509f6ac8a2ff085abbd1a9d3b0c733529bf3fcd87d43f731990467181ed1995aad5bc",
        }

        assert (await self.storage.add_event(ephemeral_event))[1]
        assert (await self.storage.get_event(ephemeral_event["id"]))[
            "id"
        ] == "2696df86ce47142b7d272408e222b7a9fc4b2cc3a428bf2debf5d730ae2f42c7"
    ...
    async def test_nip33(self):
        ...
        await self.storage.add_event(
            self.make_event(PK1, kind=31000, tags=[["d", "replace2"]])
        )
...
class APITests(BaseTestsWithStorage):
    ...
    async def get_event(
        self, ws, event_id, check=True, exists=True, req_name="checkid"
    ):
...
class MainTests(APITests):
    ...
    async def test_send_event(self):
        async with self.conductor.simulate_ws("/") as ws:
            ...
            assert data == [
                "OK",
                "0d7721e1ee4a343f623cfb86374cc2d4688784bd264b5bc26079843169f28d88",
                True,
                "",
            ]
            data = await self.send_event(ws, EVENTS[0], True)
    ...
    async def test_bad_req(self):
        async with self.conductor.simulate_ws("/") as ws:
            ...
            await ws.send_json(
                ["REQ", "toobroad", {"ids": []}, {"authors": [], "kinds": []}]
            )
            data = await ws.receive_json()
    ...
    async def test_tag_search(self):
        async with self.conductor.simulate_ws("/") as ws:
            tag_event = {
                "id": "0b5d4c896b9f8b74feb0a63f8c50536b094ae948049d4c3ab967afc98250cee3",
                "pubkey": "5faaae4973c6ed517e7ed6c3921b9842ddbc2fc5a5bc08793d2e736996f6394d",
                "created_at": 1672325827,
                "kind": 1,
                "tags": [["t", "test"], ["t", "foo"], ["h", "home"]],
                "content": "got tags",
                "sig": "fbbd5fcf0e385d39d6120735f49c52ccc9f278670d7ce0c4ff774ff454fd810c4d18c005db681bbb5754d19351b922570f18d3fa8fae697c0480ebfccbed06e7",
            }
    ...
    async def test_origin_blacklist(self):
        with self.assertRaises(errors.WebSocketDisconnected):
            async with self.conductor.simulate_ws(
                "/", headers={"origin": "http://bad.actor"}
            ):
                pass
...
class AuthTests(APITests):
    async def asyncSetUp(self):
        ...
        self.writeonly = (
            "e93505f081570221255f05341f4fbaeaf682d59d2e2472d7dd02d566f6372178",
            "647bf6e686fde33dfbfd4f3d987bc13b1c202f952b70dc3539d8d85172b40561",
            "w",
        )
        self.readwrite = (
            "9679a595bdb5fe4330a263c96201eac204dfe24b2e2dc36ac12698faf9275130",
            "1bd80e4430c40a6fa9f59582124b5a6fbc1815e26455801e6b1edf113554de03",
            "rw",
        )
        self.unknown = (
            "94cdca784fd5a951cc1a4672e86847f957f29b35b7ac9476bc891d68e5b68b34",
            "a0f1e845b43508847d10cb7ac7928cd9dafc9b82d0b3c4c08b264e177b4355de",
            "s",
        )

        for _, pubkey, role in (
            self.readonly,
            self.writeonly,
            self.readwrite,
            self.unknown,
    ...
    async def test_auth_message(self):
        ...
        async with self.conductor.simulate_ws("/") as ws:
            ...
            await ws.send_json(
                [
                    "AUTH",
                    self.make_auth_event(
                        self.unknown[0], self.readonly[1], challenge=challenge
                    ),
                ]
            )
            data = await ws.receive_json()
            ...
            await ws.send_json(
                [
                    "AUTH",
                    self.make_auth_event(
                        self.readonly[0], self.readonly[1], challenge=challenge
                    ),
                ]
            )
        ...
        async with self.conductor.simulate_ws("/") as ws:
            ...
            await ws.send_json(
                [
                    "AUTH",
                    self.make_auth_event(
                        self.writeonly[0], self.writeonly[1], challenge=challenge
                    ),
                ]
            )
        ...
        async with self.conductor.simulate_ws("/") as ws:
            ...
            await ws.send_json(
                [
                    "AUTH",
                    self.make_auth_event(
                        self.unknown[0], self.unknown[1], challenge=challenge
                    ),
                ]
            )
