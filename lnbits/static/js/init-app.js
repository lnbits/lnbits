  ...
  methods: {
    ...
    async loadDynamicContent() {
      try {
        ...
        const previousRouteName = this.$router.currentRoute.value.meta.previousRouteName;
        if (previousRouteName && window.app._context.components[previousRouteName]) {
          console.log(`Removing component for previous route: ${previousRouteName}`);
        ...
        if (!componentLogic) {
          throw new Error(`Component logic '${logicKey}' not found. Ensure it is defined in the script.`);
        }
  ...
  watch: {
    $route(to, from) {
      ...
      if (validRouteNames.includes(to.name)) {
        ...
        console.warn(`Route '${to.name}' is not valid. Leave this one to ol'Fastapi.`);
      }
    },
  },
