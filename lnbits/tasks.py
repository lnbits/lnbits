...
payment_listeners: Dict[str, asyncio.Queue] = SseListenersDict("payment_listeners")
...
def register_payment_listener(send_chan: asyncio.Queue, name: str = None):
    """
    A method intended for extensions (and core/tasks.py) to call when they want to be notified about
    new sent payments. Will emit all outgoing payments.
    """
    name_unique = f"{name or 'no_name'}_{str(uuid.uuid4())[:8]}"
    logger.trace(f"sse: registering invoice listener {name_unique}")
    payment_listeners[name_unique] = send_chan
...
async def payment_callback_dispatcher(checking_id: str):
    ...
    if payment and payment.is_in:
        logger.trace(f"sse sending callback for incoming payment {checking_id}")
        ...
        logger.trace("sending callback for outgoing payment", checking_id)
        await payment.set_pending(False)
        for chan_name, send_chan in payment_listeners.items():
            await send_chan.put(payment)
    else:
        pass
