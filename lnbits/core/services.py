...
async def create_hold_invoice(
    *,
    ...
    memo: str,
    rhash: bytes,  # Hash the HTLC of the hold invoice is locked to.
    description_hash: Optional[bytes] = None,
    extra: Optional[Dict] = None,
    webhook: Optional[str] = None,
    conn: Optional[Connection] = None,
    ...
    (
        ...
        payment_request,
        error_message,
    ) = await funding_source.create_hold_invoice(
        amount=amount,
        memo=invoice_memo,
        rhash=rhash,
        description_hash=description_hash,
    ...
    if not ok:
        raise InvoiceError(
            error_message or "Unexpected backend error.", status="failed"
        )
