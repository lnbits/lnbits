...
async def get_wallet_share(
    ...
    row = await conn.fetchone(
        "SELECT * FROM wallet_shares WHERE id = ?",
        (share_id,),
    )
...
async def get_wallet_shares(
    ...
    rows = await conn.fetchall(
        "SELECT * FROM wallet_shares WHERE wallet_id = ? ORDER BY shared_at DESC",
        (wallet_id,),
    )
...
async def get_user_shared_wallets(
    ...
    """
    rows = await conn.fetchall(
        "SELECT * FROM wallet_shares WHERE user_id = ? ORDER BY shared_at DESC",
        (user_id,),
    )
    return [WalletShare(**row) for row in rows]
...
async def update_wallet_share_permissions(
    ...
    """
    await conn.execute(
        "UPDATE wallet_shares SET permissions = ? WHERE id = ?",
        (data.permissions, share_id),
    )
    share = await get_wallet_share(conn, share_id)
...
async def delete_wallet_share(
    ...
    """
    await conn.execute(
        "DELETE FROM wallet_shares WHERE id = ?",
        (share_id,),
    )
...
async def check_wallet_share_permission(
    ...
    row = await conn.fetchone(
        ...
        WHERE wallet_id = ? AND user_id = ? AND accepted = TRUE
        """,
        (wallet_id, user_id),
    )
    if not row:
...
async def get_wallet_share_count(
    ...
    """
    row = await conn.fetchone(
        "SELECT COUNT(*) as count FROM wallet_shares WHERE wallet_id = ?",
        (wallet_id,),
    )
