...
`build.py` on macOS prepares runtimes before invoking PyInstaller. `sidecars/pins.json`
...
Builds require npm. Build on the target Mac architecture using the release workflow.
...
or sending payments. macOS CI runs it against the final read-only DMG. `smoke.py` tests the packaged
